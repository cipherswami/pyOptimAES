...
class AES:
    ...
    def encrypt_ctr(self, plaintext):
        ...
        for plaintext_block in split_blocks(plaintext):
            count += 1
            iv = self.iv[0:12] + bytes(str(count).zfill(4), 'utf-8')
            blocks.append(xor_bytes(plaintext_block, self.encrypt_aes(iv)))
    ...
    def decrypt_ctr(self, ciphertext):
        ...
        for ciphertext_block in split_blocks(ciphertext):
            count += 1
            iv = self.iv[0:12]+ bytes(str(count).zfill(4), 'utf-8')
            blocks.append(xor_bytes(ciphertext_block, self.encrypt_aes(iv)))
        return unpad(b''.join(blocks))

    def cbcmac(self, plaintext):
        plaintext = pad(plaintext)
